...
public class DiffMatchPatch {
	...
	public LinkedList<Diff> diff_main(String text1, String text2,
		...
		if (Diff_Timeout <= 0) {
			...
			deadline =
				System.currentTimeMillis() + (long) (Diff_Timeout * 1000);
		}
	...
	private String[] diff_halfMatchI(String longtext, String shorttext, int i) {
		...
		while ((j = shorttext.indexOf(seed, j + 1)) != -1) {
			...
			int suffixLength =
				diff_commonSuffix(longtext.substring(0, i),
	...
	public void diff_cleanupSemantic(LinkedList<Diff> diffs) {
		...
		while (thisDiff != null) {
			if (prevDiff.operation == Operation.DELETE
				...
				int overlap_length1 =
					this.diff_commonOverlap(deletion, insertion);
				int overlap_length2 =
					this.diff_commonOverlap(insertion, deletion);
				if (overlap_length1 >= overlap_length2) {
					if (overlap_length1 >= deletion.length() / 2.0
						...
						prevDiff.text =
							deletion.substring(0, deletion.length()
								- overlap_length1);
					...
					if (overlap_length2 >= deletion.length() / 2.0
						...
						prevDiff.text =
							insertion.substring(0, insertion.length()
								- overlap_length2);
		...
		while (nextDiff != null) {
			if (prevDiff.operation == Operation.EQUAL
				...
				if (commonOffset != 0) {
					...
					equality1 =
						equality1.substring(0, equality1.length()
							- commonOffset);
					edit =
						commonString
	...
	private int diff_cleanupSemanticScore(String one, String two) {
		...
		boolean lineBreak1 =
			whitespace1 && Character.getType(char1) == Character.CONTROL;
		boolean lineBreak2 =
			whitespace2 && Character.getType(char2) == Character.CONTROL;
			...
			if (thisDiff.operation == Operation.EQUAL) {
				...
				if (lastequality != null
					&& ((pre_ins && pre_del && post_ins && post_del) || ((lastequality
						...
						+ (pre_del ? 1 : 0) + (post_ins ? 1 : 0) + (post_del
							? 1
							: 0)) == 3))) {
					// System.out.println("Splitting: '" + lastequality + "'");
					// Walk back to offending equality.
					...
					pointer.add(thisDiff =
						new Diff(Operation.INSERT, lastequality));
			...
			case EQUAL:
				if (count_delete + count_insert > 1) {
					...
					if (both_types) {
						...
						commonlength =
							diff_commonPrefix(text_insert, text_delete);
						if (commonlength != 0) {
						...
						commonlength =
							diff_commonSuffix(text_insert, text_delete);
						if (commonlength != 0) {
							thisDiff = pointer.next();
							thisDiff.text =
		...
		while (nextDiff != null) {
			if (prevDiff.operation == Operation.EQUAL
				...
				if (thisDiff.text.endsWith(prevDiff.text)) {
					...
					thisDiff.text =
						thisDiff.text.substring(nextDiff.text.length())
							+ nextDiff.text;
					pointer.remove(); // Delete nextDiff.
					nextDiff = pointer.hasNext() ? pointer.next() : null;
					changes = true;
	...
	protected int match_bitap(String text, String pattern, int loc) {
		...
		if (best_loc != -1) {
			score_threshold =
				Math.min(match_bitapScore(0, best_loc, loc, pattern),
					score_threshold);
	...
	protected void patch_addContext(Patch patch, String text) {
		...
		// Add the prefix.
		String prefix =
			text.substring(Math.max(0, patch.start2 - padding), patch.start2);
		if (prefix.length() != 0) {
	...
	public Object[] patch_apply(LinkedList<Patch> patches, String text) {
		...
		for (Patch aPatch : patches) {
			...
			if (text1.length() > this.Match_MaxBits) {
				...
				// a monster delete.
				start_loc =
					match_main(text, text1.substring(0, this.Match_MaxBits),
						expected_loc);
				if (start_loc != -1) {
					end_loc =
						match_main(text, text1.substring(text1.length()
							- this.Match_MaxBits),
		...
		text =
			text.substring(nullPadding.length(),
	...
	public void patch_splitMax(LinkedList<Patch> patches) {
		...
		while (bigpatch != null) {
			...
			while (!bigpatch.diffs.isEmpty()) {
				...
				precontext =
					precontext.substring(Math.max(0, precontext.length()
						- Patch_Margin));
				// Append the end context for this patch.
				if (diff_text1(bigpatch.diffs).length() > Patch_Margin) {
					postcontext =
						diff_text1(bigpatch.diffs).substring(0, Patch_Margin);
				} else {
